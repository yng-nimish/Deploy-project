...
def process_parquet_file(parquet_path: str) -> pd.DataFrame:
    """
    Process a single Parquet file, reading required columns and cleaning data.
    """
    try:
        ...
        df = ddf.compute()
        print(f"Computed {len(df)} rows for {parquet_path}")
        return df
    except Exception as e:
...
def prepare_sheet(sheet_name: str, sheet_data: pd.DataFrame) -> tuple:
    """
    Prepare a single 1000x1000 sheet from sheet_data.
    """
...
async def merge_parquet_to_excel_s3(s3_input_bucket: str, s3_input_prefix: str, s3_output_bucket: str, s3_output_prefix: str = "final_output"):
    ...
    across sheets z1-z1000 using provided metadata. Processes all rows without dropping duplicates, using the last
    number for each cell. Parallelizes file reading and sheet preparation using 16 vCPUs. Writes sheets incrementally
    and triggers workbooks at 1 billion rows.
    """
    ...
    async with session.create_client('s3', config=AioConfig(connect_timeout=30, read_timeout=30)) as s3_client:
        ...
        rows_per_workbook = 1_000_000_000  # 1 billion rows
        batch_size = 100_000_000  # ~100M rows per batch
        max_workers = 16  # 16 vCPUs
                        ...
                        for future in sheet_tasks:
                            ...
                            for row_idx in range(1000):
                                for col_idx in range(1000):
                                    worksheet.write(row_idx, col_idx, sheet_df.iloc[row_idx, col_idx])
                            del sheet_df
                            gc.collect()
                            log_memory_usage()
