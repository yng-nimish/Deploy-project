...
def read_state():
    try:
        ...
        folder_index = state.get('folder_index', 24)  # Initialize to 24 if no state
        ...
        logger.info("No state file found, initializing with folder_index=24, file_count=0")
        return 24, 0
...
def processBatch(data_frame, batchId):
    try:
        ...
        # Format folder name with 4-digit padding starting from F 0024
...
dataframe_AmazonKinesis_node1753361773439 = glueContext.create_data_frame.from_options(connection_type="kinesis",connection_options={"typeOfData": "kinesis", "streamARN": "arn:aws:kinesis:us-east-1:851725381788:stream/July24th", "classification": "json", "startingPosition": "earliest", "inferSchema": "true"}, transformation_ctx="dataframe_AmazonKinesis_node1753361773439")


glueContext.forEachBatch(frame = dataframe_AmazonKinesis_node1753361773439, batch_function = processBatch, options = {"windowSize": "100 seconds", "checkpointLocation": args["TempDir"] + "/" + args["JOB_NAME"] + "/checkpoint/"})
job.commit()
