...
def rename_s3_folder(bucket_name, old_prefix, new_prefix, max_workers=20):
    ...
    try:
        ...
        for page in pages:
            if 'Contents' in page:
                objects.extend([(obj['Key'], obj['Key'].replace(
                    old_prefix, new_prefix, 1)) for obj in page['Contents']])
        ...
        logger.info(
            f"Completed: {successful}/{total_objects} objects processed successfully")
        if successful == total_objects:
            logger.info(f"Successfully renamed {old_prefix} to {new_prefix}")
        else:
            logger.warning(
                f"Partial success: {total_objects - successful} objects failed")
        ...
        if 'Contents' in response and len(response['Contents']) > 0:
            logger.warning(
                f"Old prefix {old_prefix} still contains {len(response['Contents'])} objects")
...
old_prefix = 'Batch 1/May 2/Partial 5/'
new_prefix = 'Batch 1/May 2/F 0124/'
