...
def read_state():
    try:
        ...
        folder_index = state.get('folder_index', 72)  # Initialize to 72 if no state
        ...
        logger.info("No state file found, initializing with folder_index=72, file_count=0")
        return 72, 0
...
dataframe_AmazonKinesis_node1754918472941 = glueContext.create_data_frame.from_options(connection_type="kinesis",connection_options={"typeOfData": "kinesis", "streamARN": "arn:aws:kinesis:us-east-1:851725381788:stream/Aug11th", "classification": "json", "startingPosition": "earliest", "inferSchema": "true"}, transformation_ctx="dataframe_AmazonKinesis_node1754918472941")



glueContext.forEachBatch(frame = dataframe_AmazonKinesis_node1754918472941, batch_function = processBatch, options = {"windowSize": "80 seconds", "checkpointLocation": args["TempDir"] + "/" + args["JOB_NAME"] + "/checkpoint/"})
job.commit()
