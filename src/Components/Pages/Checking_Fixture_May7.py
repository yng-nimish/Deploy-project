...
def save_to_dynamodb(serial_number, fixture_data):
    ...
    try:
        ...
        # Sort fixture_data by Mapping (A to P) and create tabular format with indentation
        sorted_data = sorted(fixture_data, key=lambda x: x['Mapping'])
        formatted_json = json.dumps(sorted_data, ensure_ascii=False, indent=2)
        # Extract folder name from PRIMARY_FOLDER (last part after splitting by '/')
        folder_name = PRIMARY_FOLDER.strip('/').split('/')[-1]
        table = dynamodb_resource.Table(CHECKING_FIXTURES_TABLE)
        item = {
            'Serial Number': serial_number,
            'FolderName': folder_name,
            'Checking_Fixture': formatted_json
        }
