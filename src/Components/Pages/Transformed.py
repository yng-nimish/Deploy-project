    ...
    with open(input_file, 'r') as file:
        for line in file:
            ...
            if len(current_chunk) >= BATCH_SIZE:
                ...
                # Calculate how many elements should be in the reshaped array
                total_elements = rows_needed * MAX_COLUMNS

                # Add NaN values if the chunk size is not a perfect multiple of MAX_COLUMNS
                current_chunk += [np.nan] * (total_elements - len(current_chunk))
        ...
        if current_chunk:
            ...
            # Add NaN values to make the chunk size a perfect multiple of MAX_COLUMNS
            current_chunk += [np.nan] * (total_elements - len(current_chunk))

            # Reshape and write the remaining chunk
            reshaped_data = np.array(current_chunk).reshape(rows_needed, MAX_COLUMNS)
            df = pd.DataFrame(reshaped_data)
            sheet_name = f'Sheet{sheet_index + 1}'
            df.to_excel(writer, index=False, header=False, sheet_name=sheet_name, startrow=0, startcol=0)
