...
def read_state():
    try:
        ...
        folder_index = state.get('folder_index', 99)
...
def processBatch(data_frame, batchId):
    try:
        ...
        for i in range(num_files):
            ...
            if file_row_count == cells_per_file:
                ...
                logger.warning(f"Batch {batchId}: Skipped file_idx {i+1} with {file_row_count} rows for {output_path}")
...
# === Kinesis Stream Source (Dynamic ARN ===
...
glueContext.forEachBatch(
    ...
    options={"windowSize": "80 seconds", "checkpointLocation": args["TempDir"] + "/" + args["JOB_NAME"] + "/checkpoint/"}
)
