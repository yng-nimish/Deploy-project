    ...
    try:
        ...
        for idx, value in enumerate(numbers):
            ...
            if row > 1000:
                row = 1
                col = 1
                if sheet_count < 1000:  # Append to same workbook until 1,000 sheets
                    sheet = workbook.create_sheet(title=f"Z{sheet_count + 1}")
                    sheet_count += 1
                    logger.info(f"Created sheet Z{sheet_count}")
                else:  # Reached 1,000 sheets, save and start new workbook
                    file_stream = BytesIO()
                ...
                # Save every 50 sheets temporarily (adjust back to 100 after testing)
                if sheet_count % 50 == 0:
                    file_stream = BytesIO()
                    workbook.save(file_stream)
                    file_stream.seek(0)
                    s3_path = f"{s3_base_path}_workbook_{workbook_count}_partial_sheets_{sheet_count}.xlsx"
                    s3_client = boto3.client('s3')
                    s3_client.put_object(Body=file_stream, Bucket="my-bucket-sun-test", Key=s3_path)
                    logger.info(f"Uploaded partial workbook {s3_path} with {sheet_count} sheets")

            # Log progress every 1M numbers (1 sheet)
            if (idx + 1) % 1000000 == 0:
                logger.info(f"Processed {idx + 1} numbers ({(idx + 1) / total_rows * 100:.2f}%)")

        # Save remaining sheets
        if row > 1 or col > 1 or sheet_count > 0:
            file_stream = BytesIO()
            workbook.save(file_stream)
            file_stream.seek(0)
            s3_path = f"{s3_base_path}_workbook_{workbook_count}.xlsx"
            s3_client = boto3.client('s3')
            s3_client.put_object(Body=file_stream, Bucket="my-bucket-sun-test", Key=s3_path)
            logger.info(f"Uploaded {s3_path} with {sheet_count} sheets")
        ...
        # Ensure logs are flushed even on failure
