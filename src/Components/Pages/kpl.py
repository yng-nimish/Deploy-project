...
# Constants
FETCH_WORKERS = 50   # Increased to match high throughput
SEND_WORKERS = 50    # Increased to parallelize Kinesis ingestion
TARGET_COUNT = 20_000_000_000  # 20 billion records in an hour
BATCH_SIZE = 500     # Max batch size allowed by Kinesis per request
QUEUE_SIZE = 2_000_000  # Larger buffer for high throughput
...
async def fetch_continuously(queue):
    """Fetch and put data into the queue."""
    async with aiohttp.ClientSession() as session:
        while True:
            # Fetch data and process concurrently
            tasks = [fetch_data_batch(session, queue) for _ in range(FETCH_WORKERS)]
            await asyncio.gather(*tasks)

async def send_to_kinesis(queue):
    """Send data to Kinesis in batches of 500 records."""
