...
# Accumulators for distributed tracking
sheets_accumulator = spark.sparkContext.accumulator(0)
numbers_accumulator = spark.sparkContext.accumulator(0)
...
def write_partition_to_excel(iterator, s3_base_path, partition_idx):
    ...
    for file_start in range(0, sheet_count, sheets_per_file):
        ...
        with pd.ExcelWriter(file_stream, engine='openpyxl') as writer:
            ...
            sheets_written = sheets_in_file
            numbers_written = min(sheets_in_file * numbers_per_sheet, total_numbers - file_start * numbers_per_sheet)
            sheets_accumulator.add(sheets_written)
            numbers_accumulator.add(numbers_written)
        file_stream.seek(0)
        s3_client.put_object(Body=file_stream, Bucket="my-bucket-sun-test", Key=output_key)
        results.append((output_key, sheets_written))
    return results
...
def save_to_excel_and_upload_to_s3(df, s3_base_path):
    ...
    sheets_accumulator.value = 0
    numbers_accumulator.value = 0
    
    results = (df.select("number")
               .rdd.mapPartitionsWithIndex(lambda idx, it: write_partition_to_excel(it, s3_base_path, idx))
               .collect())
    
    total_sheets_processed += sheets_accumulator.value
    total_numbers_processed += numbers_accumulator.value
    file_paths.extend(results)
    logger.info(f"Completed saving {numbers_accumulator.value} numbers across {len(results)} files with {sheets_accumulator.value} sheets")
...
def processBatch(data_frame, batchId):
    ...
    if initial_count > 0 and total_sheets_processed < 1000:
        ...
        transformed_df.cache()  # Cache to avoid recomputation
        ...
        transformed_df.unpersist()  # Clear cache
