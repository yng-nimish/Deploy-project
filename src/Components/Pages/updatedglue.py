...
dataframe_AmazonKinesis_node1742906686641 = glueContext.create_data_frame.from_options(connection_type="kinesis",connection_options={"typeOfData": "kinesis", "streamARN": "arn:aws:kinesis:us-east-1:851725381788:stream/GoldStream", "classification": "json", "startingPosition": "earliest", "inferSchema": "true"}, transformation_ctx="dataframe_AmazonKinesis_node1742906686641")
...
def processBatch(data_frame, batchId):
    try:
        if data_frame.count() > 0:
            ...
            logger.info(f"transformed dynamic frame has data")
            transformed_df = transformed_dynamic_frame.toDF()
            transformed_df.show(2)
            transformed_df_pandas = transformed_df.toPandas()
            logger.info(f"Converted spark data frame to pandas dataframe")
            now = datetime.datetime.now()
            s3_base_path = f"s3://my-bucket-sun-test/temp/ingest_year={now.year:0>4}/ingest_month={now.month:0>2}/ingest_day={now.day:0>2}/output"
...
glueContext.forEachBatch(frame = dataframe_AmazonKinesis_node1742906686641, batch_function = processBatch, options = {"windowSize": "100 seconds", "checkpointLocation": args["TempDir"] + "/" + args["JOB_NAME"] + "/checkpoint/", "batchMaxPollingTimeSeconds": 3600  # 1 hour
