...
def find_parquet_file(folder_path):
    ...
    try:
        ...
        logger.error(f"No parquet file matching size criteria in {folder_path-cup} or its stripped version")
        return None
...
def save_to_dynamodb(serial_number, fixture_data):
    ...
    try:
        ...
        item = {
            ...
            'FolderName': 'Test2',
            'Checking_Fixture': json.dumps(fixture_data, ensure_ascii=False)
        }
...
def main():
    ...
    try:
        ...
        # Process folders sequentially for debugging
        fixture_data = {}
        ...
        for folder in required_folders:
            ...
            fixture_data.update(results)
            logger.info(f"Completed processing folder {folder}")

        # Check if all A-P mappings are present
        missing_mappings = [key for key in MAPPINGS if key not in fixture_data]
        if missing_mappings:
